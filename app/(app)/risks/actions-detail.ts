...
if (!risk) {
  // 1) Existe esse risco em QUALQUER tenant?
  ...
  if (!any.rows[0]) {
    throw new Error(
      `Risco não existe. id=${String(riskId)} ctx.tenantId=${String(ctx.tenantId)}`
    )
  }

  throw new Error(
    `Risco existe, mas tenant mismatch. id=${String(riskId)} risco.tenant_id=${any.rows[0].tenant_id} ctx.tenantId=${String(
      ctx.tenantId
    )}`
  )
}
