...
export default function FiltersBar({
  ...
  // garante mes_ref sempre (mês atual) — mesmo padrão de Controles
  useEffect(() => {
  ...
  // =============================
  // Design tokens locais (mesmo padrão do seu Controls)
  // =============================
  const baseField =
  ...
  const hasActiveFilters = useMemo(() => {
    ...
    return Boolean(qv || framework || frequency || owner || focal || resultado || monthDiff)
  ...
  return (
    <div className="bg-white dark:bg-background-dark border border-slate-200 dark:border-slate-800 p-4 rounded-xl space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input
            ...
            placeholder="Buscar por nome, código, operador..."
            type="text"
          />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <option value="">Control owner (todos)</option>
          {opts.owners.map((o) => {
        ...
        >
          <option value="">Focal point (todos)</option>
          {opts.focals.map((o) => {
            const label = o.name && o.email ? `${o.name} (${o.email})` : o.name || o.email
            const value = o.email !== "—" ? o.email : o.name !== "—" ? o.name : ""
            if (!value) return null
