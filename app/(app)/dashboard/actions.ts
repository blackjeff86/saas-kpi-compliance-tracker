...
export async function fetchDashboardSummary(q: DashboardQuery = {}): Promise<DashboardSummary> {
  ...
  const cardsRes = await sql<{
    ...
    WITH controls_base AS (
      SELECT
        ...
        c.name,
        c.framework_id,
        c.frequency::text AS frequency_text,
        c.frequency_key::text AS frequency_key_text
      FROM controls c
  ...
  // =========================
  const wfRes = await sql<{ workflow_status: string; count: number }>`
    SELECT
      e.workflow_status::text AS workflow_status,
      COUNT(*)::int AS count
    FROM kpi_executions e
    ...
    WHERE e.tenant_id = ${tenantId}
      ...
      AND e.period_start >= ${startDate}::date
      AND e.period_start <  ${endDate}::date
    GROUP BY e.workflow_status
    ORDER BY count DESC, workflow_status ASC
  `

  // =========================
  // Execuções por auto_status (período + framework)
  // =========================
  const autoRes = await sql<{ auto_status: string; count: number }>`
    SELECT
      e.auto_status::text AS auto_status,
      COUNT(*)::int AS count
    FROM kpi_executions e
    ...
    WHERE e.tenant_id = ${tenantId}
      ...
      AND e.period_start >= ${startDate}::date
      AND e.period_start <  ${endDate}::date
    GROUP BY e.auto_status
    ORDER BY count DESC, auto_status ASC
  `

  // =========================
  // Action plans por prioridade
  // =========================
  const prioRes = await sql<{ priority: string; count: number }>`
    SELECT
  ...
  const dueSoonRes = await sql<{
    ...
    FROM action_plans ap
    LEFT JOIN controls c ON c.id = ap.control_id
    LEFT JOIN kpis k ON k.id = ap.kpi_id
    WHERE ap.tenant_id = ${tenantId}
      AND ap.status::text <> 'done'
      AND ap.due_date IS NOT NULL
  ...
  // =========================
  const perfRes = await sql<{ month: string; pct_in_target: number }>`
    WITH months AS (
    ...
    agg AS (
      SELECT
        date_trunc('month', e.period_start)::date AS month_start,
        COUNT(*)::int AS total,
        SUM(CASE WHEN e.auto_status::text = 'in_target' THEN 1 ELSE 0 END)::int AS ok
      FROM kpi_executions e
      JOIN controls c ON c.id = e.control_id
      WHERE e.tenant_id = ${tenantId}
        AND (${frameworkId}::uuid IS NULL OR c.framework_id = ${frameworkId}::uuid)
        AND e.period_start >= (date_trunc('month', CURRENT_DATE) - interval '5 months')::date
      GROUP BY 1
    )
    SELECT
      to_char(m.month_start, 'Mon')::text AS month,
      COALESCE(ROUND((a.ok::numeric / NULLIF(a.total,0)) * 100, 0), 0)::int AS pct_in_target
    FROM months m
  ...
  const criticalRes = await sql<{
    ...
    WITH latest_exec AS (
      ...
      FROM kpi_executions e
      WHERE e.tenant_id = ${tenantId}
      ORDER BY e.control_id, e.period_end DESC NULLS LAST, e.created_at DESC
    )
    SELECT
      c.id::text AS control_id,
      c.control_code::text AS control_code,
      c.name::text AS control_name,
      u.name::text AS owner_name,
      le.workflow_status::text AS workflow_status,
      le.due_date::text AS due_date
    FROM controls c
    JOIN risks r ON r.id = c.risk_id
    LEFT JOIN users u ON u.id = c.owner_user_id
    LEFT JOIN latest_exec le ON le.control_id = c.id
    WHERE c.tenant_id = ${tenantId}
      AND (${frameworkId}::uuid IS NULL OR c.framework_id = ${frameworkId}::uuid)
      AND r.classification::text IN ('high','critical')
